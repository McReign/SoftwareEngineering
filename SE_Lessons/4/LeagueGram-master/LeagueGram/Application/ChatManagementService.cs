  ...
  public class ChatManagementService : IChatManagementService
  {
    ...
    public Guid CreateChannel(Guid creatorId)
    {
      ...
      return newChannelId;
        ...
        public void UpgradeToGroup(Guid chatId, Guid actorId )
        {
            var chat = _chatRepository.LoadChat(chatId);
            var privateChat = chat as PrivateChat;
            if (privateChat != null)
            {
                var group = privateChat.ToGroup(actorId);
                _chatRepository.SaveChat(group);
            }
            else throw new Exception();
        }

        public IEnumerable<IChat> GetUserChats(Guid userId)
        {
